 ...
 */
const LobbyPage = ({ initialLobbyData }: { initialLobbyData: Game }) => {
  const router = useRouter();
  const { id: lobbyId } = router.query;
  const username =
    typeof window !== "undefined" ? localStorage.getItem("username") : null;

  const [lobby, setLobby] = useState<Game | null>(initialLobbyData);
  ...
  const determineWinner = () => {
    ...
    const allScored = lobby.players.every((player: string) => {
      ...
      return scores
        ? ALL_CATEGORIES.every((category) => scores[category] !== undefined)
        : false;
    });
  ...
  const canJoin =
    !lobby.started && lobby.players && !lobby.players.includes(username || "");
  const canStart =
    lobby.host === username && !lobby.started && lobby.players.length >= 2;
      ...
      {lobby.started ? (
        <div className={styles["game-layout"]}>
          <div className={styles["game-main"]}>
            <div
              className={`${styles["game-section"]} ${styles["dice-section"]}`}
            >
              <Dice
...
export const getServerSideProps = async (context: any) => {
  ...
  try {
    const response = await fetch(
      `http://localhost:3000/api/games?gameId=${id}`
    );
    if (!response.ok) {
      throw new Error("Failed to fetch lobby data");
    }
    const initialLobbyData = await response.json();
